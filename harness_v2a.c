	...
	enum oil_fmt *fmt)
{
	*opts = 0;
	*gray = 0;
	if (!strcmp(name, "G"))    { *cmp = 1; *gray = 1; *fmt = OIL_GREYSCALE;       return 1; }
	if (!strcmp(name, "GA"))   { *cmp = 2; *gray = 1; *fmt = OIL_GREYSCALE_ALPHA; return 1; }
	if (!strcmp(name, "RGB"))  { *cmp = 3;            *fmt = OIL_RGB;             return 1; }
	if (!strcmp(name, "RGBX")) { *cmp = 4; *opts = 1; *fmt = OIL_RGBX;            return 1; }
	if (!strcmp(name, "RGBA")) { *cmp = 4;            *fmt = OIL_RGBA;            return 1; }
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if (!parse_cs(argv[2], &cmp, &opts, &gray, &fmt)) {
