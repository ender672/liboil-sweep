...
- A benchmark input PNG, **RGBA**, passed as the sole positional argument to `sweep.sh`. Any reasonably-sized real-world photo works; the loader rejects non-RGBA inputs (`harness_png.h:45`).
...
```sh
# Full sweep:
LIBOIL_REPO=/path/to/liboil ./sweep.sh /path/to/img.png

# More iterations per rev (takes min across runs — reduces timing noise):
LIBOIL_REPO=/path/to/liboil OILITERATIONS=100 ./sweep.sh /path/to/img.png

# Re-run a subset; merges results into an existing benchmarks.csv:
LIBOIL_REPO=/path/to/liboil SWEEP_RANGE="A..B" ./sweep.sh /path/to/img.png
